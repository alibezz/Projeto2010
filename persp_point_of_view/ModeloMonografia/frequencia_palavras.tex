...
%amarrar essa coisa de contagem de palavras

%A classificação de um documento de acordo com sua perspectiva é o foco deste projeto. Embora alguns trabalhos revisados, como \textbf{X e Y}, utilizem aspectos sintáticos ou semânticos dos documentos para identificar suas perspectivas, a simples consideração de suas contagens de palavras, muitas vezes, é suficiente para se atingir um bom desempenho. \textbf{fale estatisticas. fale do qto melhora de fato usar outras coisas. fale de } Nestes casos, podem ser utilizados os classificadores apresentados no Capítulo \ref{basicos}. No caso do Naïve Bayes, tem-se que, por padrão, ele já utiliza apenas as  contagens de palavras dos documentos. De todo modo, é possível alterar as representações dos documentos e do vocabulário \ensuremath{V} (ver seção \ref{subsection:naive}) para adaptar a classificação a contagens de outras propriedades dos documentos, como classe gramatical das palavras (sintaxe) ou polaridade dos termos (semântica). Não foi encontrado, entretanto, nenhum trabalho que utilize o Naïve Bayes de formas diferentes daquela apresentada na seção \ref{subsection:naive}. No caso dos SVMs, os documentos são representados como vetores em um espaço euclidiano (ver seção \ref{subsection:SVMs}). Em todos os trabalhos revisados neste capítulo, que fazem uso de SVMs, as entradas desses vetores correspondem exclusivamente a suas contagens de palavras. Há, entretanto, alguns trabalhos que atribuem entradas desses vetores a contagens de outras propriedades dos documentos. O trabalho de %Trabalhos que utilizam outras propriedades dos documentos em suas representações vetoriais

% dos documentos

%As duas principais vantagens em se utilizar uma classificação baseada Uma vantagem de se utilizar apenas contagens de palavras %- mais simples de implementar e mais fácil de adaptar para documentos em outras línguas - %, a partir de propriedades dos documentos, classificadores como Naïve Bayes ou SVMs devem definir a perspectiva de cada um deles. Embora seja possível considerar aspectos sintáticos e semânticos desses documentos no conjunto dessas propriedades, ou ainda analisar como eles citam


Partindo da hipótese de que documentos escritos sob perspectivas diferentes costumam enfatizar termos distintos \cite{teubert}, classificadores podem ser empregados para, considerando apenas as diferentes ocorrências de palavras em documentos, identificar suas perspectivas. O número de ocorrências de uma palavra em um documento é comumente denominado de \textbf{contagem}, e grande parte dos artigos estudados para esta monografia classificam textos baseando-se nessa informação. De treze trabalhos que envolvem classificação, nove se baseiam em contagens de palavras - seis exclusivamente e três combinando essa informação com outras propriedades dos documentos. Pela relevância que a contagem de palavras tem na Mineração de Perspectiva, portanto, esse capítulo é dedicado à revisão e discussão de seu uso na identificação automática da perspectiva de documentos.


Variações no uso de contagem de palavras foram encontradas em parte dos trabalhos estudados para este projeto: alguns deles utilizam valores \emph{booleanos} para indicar a presença (1) ou ausência (0) de palavras nos documentos, como o estudo de Klebanov, Beigman e Diermeier \cite{klebanov}, e outros empregam  as contagens normalizadas em relação ao corpus, como o trabalho de Hirst, Riabinin e Graham \cite{hirst-et-al} De todo modo, a hipótese linguística assumida por esses trabalhos é a mesma: textos escritos sob perspectivas diferentes empregam palavras de forma distinta. %\footnote{Essas outras características aparecem ora combinadas entre si, ora associadas à contagem de palavras padrão, ora separadamente.}.

%ou mesmo utilizar palavras com conotações diferentes para um mesmo propósito,
% em seus métodos, mesmo q n exclusivamente, ou comparam o uso dessa carac. com aquilo q desenvolvem.
%usam tb contagens, dentro do método: 3 / usam contagens pra comparar com o mét. principal: 2 / usa só contagens: 6 / 
% ainda que combinem, eventualmente, os resultados dos classificadores a outros métodos ou utilizem também outras características dos documentos.
...
%Para não tornar a leitura desse capítulo cansativa, e não diminuir o nível de detalhamento das revisões apresebtadas nessa seção, decidiu-se que, dentre os seis trabalhos estudados que classificam documentos baseando-se apenas em contagens de palavras, apenas os dois mais citados serão introduzidos nesta seção\footnote{Os números de citações foram verificados no dia 22 de setembro de 2010, com auxílio do Google Scholar (http://scholar.google.com).}. Os outros 

Dentre os seis trabalhos que se baseiam exclusivamente em contagens de palavras, o artigo de Lin et Al. é o único a utilizar um classificador diferente de um Naïve Bayes ou SVM, introduzindo o método \emph{Latent Sentence Perspective Model} (LSPM) \cite{lin-et-al2006}. O artigo de Mullen e Malouf, por sua vez, é o \textbf{único} que não atinge uma boa taxa de acerto na classificação \cite{aaai-politics}. Esses artigos são apresentados, nessa ordem, nas subseções \ref{sec:lin-et-al} e \ref{sec:aaai}. %O primeiro deles visa a classificar %De certa forma, isso fomenta as discussões sobre escolha de palavras e classificação, apresentadas na seção \ref{freqs:experim}.

%, dentre seis artigos que classificam documentos baseando-se apenas em contagens de palavras,
%Cinco dos seis trabalhos que classificam documentos baseando-se apenas em contagens de palavras obtêm bons resultados. Dentre aqueles apresentados nessa seção, entretanto, um é justamente aquele que não atinge uma boa taxa de acerto. De certa forma, isso fomenta as discussões sobre escolha de palavras e classificação, apresentadas na seção \ref{freqs:experim}. O outro, além de apresentar bons resultados, é o único a utilizar um classificador diferente de Naïve Bayes ou SVMs: ele introduz um novo método, denominado \emph{Latent Sentence Perspective Model} (LSPM).%  O critério para escolhê-los foi o número de citações por outros trabalhos, por uma questão de relevância para a área de Mineração de Perspectiva. % decidiu-se que os dois artigos revisados seriam os mais citados,  - e%cl % para classificadizer q cont. de palavras é massa e mtas vezes funciona.dar exemplos. como n há espaço e vc quer fomentar a discussão sobre qdo dá errado, vc escolheu dois pra revisar em detalhes: um q desse errado e outro certo. a partir disso, escolheu os mais citados q obedecessem a esses critérios.
