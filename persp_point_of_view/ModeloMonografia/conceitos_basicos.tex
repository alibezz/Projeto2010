...
A \textbf{validação cruzada de \ensuremath{k} dobras} é uma técnica estatística que evidencia o quanto uma classificação generaliza para um conjunto aleatório de documentos \cite{}. A técnica é associada a classificadores que permitem o uso de conjuntos de treinamento e teste em suas metodologias - caso dos SVMs, do Naïve Bayes e de outros métodos estudados para esse projeto, apresentados nos Capítulos \ref{chap3} e \ref{outros}. Nesse tipo de validação, particiona-se, aleatoriamente, um conjunto de documentos \ensuremath{D} em \emph{k} subconjuntos, denominados \textbf{dobras}\footnote{Para os experimentos realizados neste projeto, cada dobra tem aproximadamente a mesma cardinalidade.}. O conjunto de teste corresponde a uma das \ensuremath{k} dobras e as \ensuremath{k - 1} restantes, unidas, compõem o conjunto de treinamento.

A classificação deve ser executada \ensuremath{k} vezes, com uma dobra diferente como conjunto de teste por vez. As métricas associadas ao desempenho de cada classificação podem ser consideradas conjuntamente, através de uma média aritmética, ou em separado \cite{}. Os resultados obtidos evidenciam o quanto a classificação conserva seu desempenho, independentemente do conjunto de teste selecionado. Para os trabalhos estudados nessa monografia, os valores mais comuns para \ensuremath{k} foram cinco e dez. % É interessante ressaltar que, na validação cruzada de \ensuremath{k} dobras, todos os documentos de \ensuremath{D} são utilizados tanto no conjunto de treinamento quanto no de teste. Além disso,  
