...
%algo decisivo para a escolha dos veículos em que escrevem

%o que tornou a escolha deste tipo de veículo mais conveniente do que a busca e análise 
...
%dos principais candidatos à sucessão presidencial - Dilma Rousseff, pelo PT, José Serra, pelo PSDB, e Marina Silva, pelo PV\footnote{Na data em que esta passagem foi escrita, 16/09/2010, a pesquisa sobre intenções de voto mais recente, divulgada pelo CNT/Sensus em 14/09/2010, informou que a candidata do PT possuía 50,5\% das intenções contra 26,4\% do candidato do PSDB e 8,9\% da candidata do PV.} - disponíveis na \emph{Web}.

%analisam as perspectivas contidas em documentos sobre governos federais
...
%NOMEAR CORPUS
%Na maioria dos artigos revisados para este projeto, os corpora analisados são divididos em dois lados, sustentando pontos de vista antagônicos sobre um mesmo tema. Para o corpus construído para este estudo de caso, a mesma estratégia foi adotada: o tema geral é o governo Lula e a sucessão presidencial, e os artigos dividem-se entre pró e anti governo. 


%Como o estudo visa à mineração das perspectivas pró/anti-governo federal, um critério básico para a escolha das colunas e \emph{sites} foi a existência de uma posição clara de crítica ou apoio ao atual governo. A notoriedade dos autores também foi considerada, facilitando a integração deste estudo a análises sobre a cobertura das Eleições 2010 em \textbf{TERMINE ISSO, e fale pq n políticos, e sim jornalistas}.

%Este corpus, portanto, assim como outros revisados nesta monografia (vide capítulos \textbf{X} e \textbf{Y}), contém duas perspectivas em oposição sobre um mesmo assunto: o governo federal brasileiro. 
\begin{enumerate}
...
%Os estudos de Mullen e Malouf com um fórum de discussão política também sugerem que a contribuição dos participantes é assimétrica, obedecendo a uma distribuição livre de escala
...
%como evidencia a Tabela % adicionalmente, também enfatizam % No segundo caso,  % diferentes perspectivas, bem como 
 % descoberta automática de seus posicionamentos. %Esta discussão, entretanto, envolve conhecimentos mais apurados de Comunicação e Ciência Política, fugindo do escopo deste projeto. %sendo apenas provocada - e,  fugir do escopo deste projeto, 

%Dado que eles são jornalistas de notoriedade nacional, cujos textos, em parte dos casos, também foram publicados \emph{online} em revistas e jornais de alta circulação (como Veja e O Globo), a alta taxa de acerto obtida neste estudo, que evidencia , 


%Apesar do número de veículos analisados ser um tanto pequeno, esses resultados provocam a discussão sobre a formação de opinião na mídia brasileira \emph{online} - não apenas porque os jornalistas possuem muita notoriedade ou escrevem para revistas e jornais de alta circulação, como Veja e O Globo, mas também por eles serem muito lidos e comentados. Muito comumente, artigos dos autores selecionados receberam mais de cem comentários em suas colunas e \emph{blogs}.  % FALAR DE COMO TEM MTO COMMENT NOS BLOGS/COLUNAS MESMO!! Apesar dos veículos escolhidos para este estudo não representarem adequadamente %a mídia política online

%discutir o l-lda, marina silva n aparecer
